...
def test_p22_8_acceptance_preserves_authority_and_activation_boundaries():
    ...
    roadmap_version = next(line for line in roadmap.splitlines() if line.startswith("Version: ")).split(": ", 1)[1]
    major, minor = map(int, roadmap_version.split("."))
    assert major == 4 and minor >= 59
